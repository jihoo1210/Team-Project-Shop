...
public class ItemController {
     ...
     */
    @GetMapping
    public ResponseEntity<?> indexItem(@AuthenticationPrincipal CustomUserDetails customUserDetails,
        ...
        try {
            ...
            User user = customUserDetails.getUser(); // customUserDetails null 체크 필요
     ...
     */
    @GetMapping("/favorite")
    public ResponseEntity<?> indexFavorite(@AuthenticationPrincipal CustomUserDetails customUserDetails,
                            @PageableDefault(size = 10, sort = "id", direction = Direction.DESC) org.springframework.data.domain.Pageable pageable,
                            IndexItemRequest searchParams) {
        try {
            // customUserDetails가 null일 경우 NPE 발생 가능
            // user 객체가 null일 경우 서비스에서 예외 발생 가능
    ...
    @GetMapping("/cart")
    public ResponseEntity<?> indexCart(@AuthenticationPrincipal CustomUserDetails customUserDetails,
                            @PageableDefault(size = 10, sort = "id", direction = Direction.DESC) org.springframework.data.domain.Pageable pageable,
                            IndexItemRequest searchParams) {
        try {
            // customUserDetails가 null일 경우 NPE 발생 가능
     ...
     */
    @GetMapping("/{itemId}")
    public ResponseEntity<?> showItem(@AuthenticationPrincipal CustomUserDetails customUserDetails, @PathVariable Long itemId) {
        try {
            User user = customUserDetails.getUser();
    ...
    public ResponseEntity<?> toggleFavorite(@AuthenticationPrincipal CustomUserDetails customUserDetails, @PathVariable Long itemId) {
        try {
            User user = customUserDetails.getUser();
    ...
    public ResponseEntity<?> toggleCart(@AuthenticationPrincipal CustomUserDetails customUserDetails, @PathVariable Long itemId) {
        try {
            User user = customUserDetails.getUser();
